...
const CartList = () => {
  ...
  const handleIncreaseQuantity = async (productId) => {
    try {
      const updatedProduct = products?.cartItems?.find(
      ...
      if (updatedProduct) {
        ...
        // In giá trị updatedQuantity ra màn hình console
        console.log("Updated Quantity:", updatedQuantity);

        // Gọi action để cập nhật sản phẩm trong Redux store
        dispatch(updateCartItem({ quantity: updatedQuantity }, pro));
      ...
      toast.error("Có lỗi khi cập nhật số lượng sản phẩm");
    }
  };
  ...
  const handleDeCreaseQuantity = async (productId) => {
    try {
      const updatedQuantity = Math.max(
        products?.cartItems?.find((product) => product.id === productId)
          ?.quantity - 1,
        1
      );

      // Gọi action để cập nhật sản phẩm trong Redux store
      dispatch(updateCartItem({ quantity: updatedQuantity }, productId));
