...
const CartList = () => {
  ...
  const handleIncreaseQuantity = async (productId) => {
    try {
      const updatedQuantity = products.cartItems.find(
  ...
  const handleDeCreaseQuantity = async (productId) => {
    try {
      const updatedQuantity = Math.max(
        products.cartItems.find((product) => product.id === productId)?.quantity -
          1,
        1
      );

      // Gọi action để cập nhật sản phẩm trong Redux store
      dispatch(apiUpdaupdateCartItem(productId, { quantity: updatedQuantity }));
